...
"background-color: rgb(78, 78, 78);")
    ...
    def CreateDB(self):
        # Ensure the NEWBANK table exists
    ...
    def insertdb(self):
        ...
        if '@' not in email or '.' not in email:
            self.general_message('Invalid Email', 'Please enter a valid email address.')
            return
        ...
        if password != confirmPass:
            ...
            return
        if len(password) < 6:
            self.general_message('Password Error', 'Password must be at least 6 characters long.')
            return

        # Validate phone number
        if not phone.isdigit() or len(phone) > 15:
            self.general_message('Invalid Phone Number', 'Phone number must be up to 15 digits.')
            return

        # Insert data into the database
        try:
            c.execute("INSERT INTO NEWBANK (USERNAME, FIRSTNAME, LASTNAME, EMAIL, PASSWORD, CONFIRM, PHONE, SEX, ADDRESS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                      (username, firstname, lastname, email, password, confirmPass, phone, sex, address))
            dbb.commit()
            self.general_message('Success', 'Registration successful!')
            self.login()
        except sqlite3.IntegrityError as e:
            if 'UNIQUE constraint' in str(e):
                self.general_message('Registration Error', 'Username, email, or phone number already exists.')
            else:
                self.general_message('Database Error', str(e))
